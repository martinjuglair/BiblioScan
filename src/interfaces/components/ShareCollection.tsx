...
export function ShareCollection({ books, categoryCount, onClose }: ShareCollectionProps) {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `shelfy-collection-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
...
async function generatePDF(
  ...
  doc.text("Shelfy", pageW / 2, 28, { align: "center" });
  doc.setFontSize(12);
  ...
  doc.setTextColor(...gray);
  doc.setFontSize(7);
  doc.text(`Shelfy \u2014 ${dateStr} \u2014 ${books.length} livres`, pageW / 2, y, { align: "center" });

  // Save
  doc.save(`shelfy-collection-${new Date().toISOString().slice(0, 10)}.pdf`);
}

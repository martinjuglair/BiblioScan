...
import { fnacAffiliateUrl } from "@interfaces/utils/affiliate";
...
export function GroupDetail({ groupId, onBack }: GroupDetailProps) {
  ...
  const handleCopyInvite = async () => {
    if (!group) return;
    hapticMedium();
    const deepLink = `ploom://join/${group.inviteCode}`;
    const text = `Rejoins mon groupe de lecture "${group.name}" sur Ploom !\n\n👉 ${deepLink}\n\nOu entre le code : ${group.inviteCode}`;
    if (navigator.share) {
      try {
      ...
      <div className="flex gap-2 mb-4">
          ...
          {isAdmin && (
            ...
            >
              ...
              <span className="text-sm font-semibold text-text-primary">Réglages</span>
            </button>
          )}
        </div>
        ...
        <div className="space-y-4 pb-4">
          ...
          {selectedBook && (
            <div className="space-y-2">
              ...
              <a
                href={fnacAffiliateUrl({ isbn: selectedBook.isbn, title: selectedBook.title })}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-[#E4B80844] bg-[#FFF6E6] text-[#B57800] font-semibold text-sm transition-all active:scale-[0.97]"
              >
                🛒  Acheter sur Fnac
              </a>
            </div>

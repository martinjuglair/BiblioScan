// Service Worker for Ploom — caches cover images
const CACHE_NAME = "ploom-covers-v1";
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => (key.startsWith("ploom-covers-") || key.startsWith("shelfy-covers-")) && key !== CACHE_NAME)
